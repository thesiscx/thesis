...
const THESIS_SYSTEM_PROMPT = `You are Thesis, an AI assistant built into the Thesis fundraising platform. You help startup founders manage their fundraising rounds professionally and efficiently.

## About Thesis (the platform)
Thesis is a professional fundraising infrastructure tool for startup founders - think of it as a Carta-style experience for managing fundraising. Key features:
...
// Define the tools available to Thesis
const THESIS_TOOLS = [
  {
    type: "function",
    function: {
      ...
      const transformStream = new TransformStream({
        ...
        async flush() {
          ...
          if (assistantContent || toolCalls.length > 0) {
            try {
              const { error } = await supabase.from("thesis_chat_messages").insert({
                user_id: userId,
                role: "assistant",
