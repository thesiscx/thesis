...
import thesisSplashGif from "@/assets/thesis-splash-new.gif";

interface ThesisSplashProps {
  onComplete: () => void;
}

export function ThesisSplash({ onComplete }: ThesisSplashProps) {
  ...
  useEffect(() => {
    const img = new Image();
    img.src = `${thesisSplashGif}?t=${gifKey.current}`;
  ...
  return (
    ...
    >
      <img 
        src={`${thesisSplashGif}?t=${gifKey.current}`}
        alt=""
